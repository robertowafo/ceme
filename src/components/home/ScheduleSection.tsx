    ...
    <section ref={root} className="bg-cream py-20 sm:py-28">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-14">
          <span className="block text-burgundy text-xs font-semibold uppercase tracking-[0.2em] mb-4">Rejoignez-nous</span>
          <h2 className="font-serif text-3xl sm:text-5xl font-extrabold text-soft-black leading-tight">
            Des rendez-vous réguliers, <span className="text-gold italic">ouverts à tous.</span>
          </h2>
        </div>
          ...
          {cards.map((c) => (
            <div
              ...
              className={`schedule-card rounded-3xl p-8 sm:p-10 border ${
                c.highlight ? 'bg-soft-black text-white border-gold/40' : 'bg-white text-soft-black border-soft-black/10'
              }`}
            >
              ...
              <div className="flex items-baseline gap-3 mt-6 mb-2">
                <span className="font-serif text-5xl font-extrabold text-gold">{c.day}</span>
              </div>
              ...
              <Link
                to={c.to}
                className="mt-8 inline-flex w-full items-center justify-center bg-gold hover:bg-yellow-500 text-soft-black px-6 py-3.5 rounded-full font-sans font-bold text-sm uppercase tracking-wider transition-colors"
              >
                {c.cta}
              </Link>
