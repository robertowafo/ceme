...
import { Heart, Send, ShieldCheck, Users, Radio, BookOpen, Quote, Phone, Loader2, MessageSquare, Sparkles } from 'lucide-react';
import { submitPrayerRequest, getPublicPrayerRequests, PrayerRequestPublic } from '../lib/dbService';
import { SEO } from '../components/SEO';
import { breadcrumbSchema } from '../lib/structuredData';

const promises = [
  { ref: 'Matthieu 7:7',    text: '"Demandez, et l\'on vous donnera; cherchez, et vous trouverez; frappez, et l\'on vous ouvrira."',     bg: 'bg-grace-blue-deep' },
  { ref: 'Philippiens 4:6', text: '"Ne vous inquiétez de rien, mais faites connaître vos besoins à Dieu par des prières et supplications."', bg: 'bg-grace-indigo' },
  { ref: 'Jérémie 33:3',    text: '"Invoque-moi, et je te répondrai; je t\'annoncerai de grandes choses que tu ne connais pas."',          bg: 'bg-[#0f2a3d]' },
  { ref: '1 Jean 5:14',     text: '"Si nous lui demandons quelque chose selon sa volonté, il nous écoute."',                               bg: 'bg-grace-blue-deep' },
];
...
export function PrayerGraceTV() {
  ...
  return (
    <div className="bg-cream min-h-screen">
      <SEO
        title="Prière — Grâce TV"
        description="Confiez votre demande de prière à l'équipe d'intercession de Grâce TV et de la Chapelle de l'Éternel Mon Étendard."
        path="/priere"
      ...
      {/* ====================================================== HERO */}
      <div className="relative bg-grace-blue-deep text-white py-32 px-4 overflow-hidden">
        ...
        <div className="absolute bottom-0 right-0 w-[500px] h-[500px] bg-grace-sky/15 rounded-full blur-[140px]" />
        <div className="max-w-4xl mx-auto text-center relative z-10 mt-16">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9 }}>
            <div className="w-20 h-20 rounded-full bg-grace-orange/15 border border-grace-orange/30 flex items-center justify-center mx-auto mb-8">
              <Heart className="w-10 h-10 text-grace-orange" />
            </div>
            <span className="inline-flex items-center gap-2 text-grace-orange text-xs font-semibold uppercase tracking-[0.25em] mb-6">
              <Sparkles className="w-4 h-4" /> Intercession Grâce TV
            </span>
            <h1 className="font-serif text-5xl md:text-7xl font-bold mb-8 text-white leading-tight">
              La Prière,<br />
              <span className="text-grace-orange italic">Notre Force</span>
            </h1>
            <div className="text-xl text-white/70 font-light italic max-w-2xl mx-auto leading-relaxed border-l-4 border-grace-orange pl-6 text-left">
              "Ne vous inquiétez de rien; mais en toute chose faites connaître vos besoins à Dieu par des prières et des supplications, avec des actions de grâces."
              <span className="font-bold text-sm not-italic block mt-3 text-grace-orange uppercase tracking-widest">— Philippiens 4:6</span>
            </div>
          </motion.div>
        </div>
      </div>
      ...
      <div className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
            <p className="text-grace-orange font-bold uppercase tracking-widest text-sm mb-4">Fondement</p>
            <h2 className="font-serif text-4xl font-bold mb-4 text-soft-black">Pourquoi la Prière Est Essentielle</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              { icon: Users,       title: 'Une Équipe Dédiée',       text: 'Des intercesseurs formés se relaient chaque semaine pour prier spécifiquement pour les requêtes soumises. Vous n\'êtes pas seul.', featured: true  },
              { icon: Radio,       title: 'Portée par Grâce TV',     text: 'Chaque requête rejoint la même équipe d\'intercession que celle de l\'église porteuse — la CEME. Une seule prière, une seule famille.',            featured: false },
            ].map((item, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                className={`p-8 rounded-3xl text-center relative ${item.featured ? 'bg-grace-blue-deep text-white shadow-2xl border border-grace-orange/20' : 'bg-cream border border-transparent hover:border-grace-orange/20 transition-colors'}`}>
                {item.featured && (
                  <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-grace-orange text-white text-[10px] font-bold uppercase tracking-widest px-3 py-1 rounded-full whitespace-nowrap">Intercession</span>
                )}
                <div className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-6 ${item.featured ? 'bg-white/10' : 'bg-white shadow-sm'}`}>
                  <item.icon className={`w-8 h-8 ${item.featured ? 'text-grace-orange' : 'text-grace-blue'}`} />
                </div>
                <h3 className={`font-serif text-xl font-bold mb-3 ${item.featured ? 'text-grace-orange' : 'text-soft-black'}`}>{item.title}</h3>
      ...
      {/* ====================================================== PROMISES */}
      <div className="py-24 bg-grace-blue-deep relative overflow-hidden">
        <div className="absolute inset-0 cross-pattern-dark opacity-25" />
        <div className="absolute top-0 left-0 w-64 h-64 bg-grace-orange/10 rounded-full blur-[80px]" />
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-14">
            <p className="text-grace-orange font-bold uppercase tracking-widest text-sm mb-4">La Parole de Dieu</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {promises.map((p, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                className={`${p.bg} rounded-3xl p-7 relative overflow-hidden border border-white/5`}>
                ...
                <p className="text-grace-orange font-bold text-xs uppercase tracking-widest border-t border-white/10 pt-4">{p.ref}</p>
              </motion.div>
            ))}
          </div>
        </div>
          ...
          <div className="flex flex-col lg:flex-row gap-12 lg:gap-16">
            ...
            <div className="lg:w-1/2">
              <div className="bg-white rounded-3xl shadow-xl border border-gray-100 relative overflow-hidden">
                <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-grace-orange to-grace-blue" />
                ...
                <div className="flex border-b border-gray-100 mt-2">
                  <button
                    ...
                    className={`flex-1 py-5 text-sm font-bold uppercase tracking-widest transition-colors relative ${
                      activeTab === 'prayer' ? 'text-soft-black' : 'text-gray-400 hover:text-gray-600'
                    }`}
                  >
                    Prières
                    {activeTab === 'prayer' && (
                      <motion.div layoutId="gtv-tab-indicator" className="absolute bottom-0 left-0 w-full h-0.5 bg-soft-black" />
                    )}
                  </button>
                  <button
                    onClick={() => switchTab('testimony')}
                    className={`flex-1 py-5 text-sm font-bold uppercase tracking-widest transition-colors relative ${
                      activeTab === 'testimony' ? 'text-grace-orange' : 'text-gray-400 hover:text-gray-600'
                    }`}
                  >
                    Témoignages
                    {activeTab === 'testimony' && (
                      <motion.div layoutId="gtv-tab-indicator" className="absolute bottom-0 left-0 w-full h-0.5 bg-grace-orange" />
                    )}
                  </button>
                </div>
                ...
                <div className="p-8 lg:p-10">
                  <AnimatePresence mode="wait">
                    {activeTab === 'prayer' ? (
                      <motion.div key="prayer-form" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} transition={{ duration: 0.2 }}>
                        <BookOpen className="w-8 h-8 text-soft-black mb-4" />
                        <h3 className="font-serif text-3xl font-bold mb-2 text-soft-black">Soumettre une Prière</h3>
                        ...
                        <form className="space-y-6" onSubmit={handleSubmit}>
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                              ...
                              <input
                                ...
                                className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-grace-orange focus:bg-white w-full transition-all"
                              />
                            </div>
                            <div>
                              <label className="block text-xs font-bold text-gray-700 uppercase tracking-wide mb-2">
                          ...
                          <div>
                            ...
                            <textarea
                              ...
                              className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-grace-orange focus:bg-white w-full resize-none transition-all"
                            />
                          </div>

                          <div className="bg-cream p-4 rounded-xl flex items-start gap-3 border border-gray-200">
                            <input
                              type="checkbox" id="gtv-confidential" checked={isConfidential} onChange={e => setIsConfidential(e.target.checked)}
                              className="accent-grace-orange w-5 h-5 shrink-0 mt-0.5"
                            />
                            <label htmlFor="gtv-confidential" className="text-sm text-gray-700 cursor-pointer">
                          ...
                          <motion.button
                            whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} type="submit" disabled={isSending}
                            className="bg-grace-blue-deep text-white hover:bg-grace-orange w-full py-4 rounded-xl font-bold uppercase tracking-wider flex items-center justify-center gap-2 transition-colors shadow-lg disabled:opacity-60 disabled:cursor-not-allowed"
                          >
                      ...
                      <motion.div key="testimony-form" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} transition={{ duration: 0.2 }}>
                        <Heart className="w-8 h-8 text-grace-orange mb-4 fill-grace-orange/20" />
                        <h3 className="font-serif text-3xl font-bold mb-2 text-soft-black">Partager un Témoignage</h3>
                        ...
                        <form className="space-y-6" onSubmit={handleSubmit}>
                          ...
                          <div>
                            ...
                            <textarea
                              rows={6} required value={message} onChange={e => setMessage(e.target.value)}
                          ...
                          <div className="bg-amber-50 p-4 rounded-xl flex items-start gap-3 border border-amber-100">
                            <Heart className="w-5 h-5 text-grace-orange shrink-0 mt-0.5 fill-grace-orange/30" />
                          ...
                          <motion.button
                            whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} type="submit" disabled={isSending}
                            className="bg-grace-orange text-white hover:bg-grace-orange-dark w-full py-4 rounded-xl font-bold uppercase tracking-wider flex items-center justify-center gap-2 transition-colors shadow-lg disabled:opacity-60 disabled:cursor-not-allowed"
                          >
            ...
            <div className="lg:w-1/2 flex flex-col">
              <div className="flex items-center justify-between mb-8">
                <h2 className="font-serif text-3xl font-bold flex items-center gap-3 text-soft-black">
                  <Radio className="w-8 h-8 text-grace-orange" />
                  Mur d'Intercession
                </h2>
                <span className="bg-white px-3 py-1 rounded-full text-xs font-bold text-grace-blue shadow-sm border border-gray-100">
                ...
                {isLoadingWall ? (
                  ...
                  wallPrayers.map((req, i) => (
                    <motion.div
                      ...
                      className="p-6 bg-white border border-gray-100 shadow-sm hover:shadow-md transition-shadow rounded-2xl relative"
                    >
                      ...
                      {req.adminReply && (
                        <div className="mt-4 bg-grace-orange/5 border border-grace-orange/20 rounded-xl p-4">
                          <div className="flex items-center gap-1.5 mb-2">
                            <MessageSquare className="w-3.5 h-3.5 text-grace-orange" />
                            <span className="text-[10px] font-bold text-grace-orange uppercase tracking-widest">Réponse de l'équipe pastorale</span>
                          </div>
      ...
      <div className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-14">
            <p className="text-grace-orange font-bold uppercase tracking-widest text-sm mb-4">La Prière Exaucée</p>
            <h2 className="font-serif text-4xl font-bold text-soft-black">Dieu Répond</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {testimonies.map((t, i) => (
                <motion.div
                  ...
                  className="bg-cream rounded-3xl p-8 border border-transparent hover:border-grace-orange/20 transition-colors group"
                >
                  <div className="flex items-center justify-between mb-6">
                    <span className="inline-block bg-grace-orange/10 text-grace-orange text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-full border border-grace-orange/20">
                      Témoignage
                    </span>
                    <span className="text-[10px] uppercase tracking-wider text-gray-400">{timeAgo(t.submittedAt)}</span>
                  </div>
                  <Quote className="w-7 h-7 text-grace-orange/30 mb-4" />
                  <p className="text-gray-700 leading-relaxed italic text-sm mb-6">"{t.message}"</p>
                  {t.adminReply && (
                    <div className="bg-white border border-grace-orange/20 rounded-xl p-4 mb-4">
                      <div className="flex items-center gap-1.5 mb-2">
                        <MessageSquare className="w-3.5 h-3.5 text-grace-orange" />
                        <span className="text-[10px] font-bold text-grace-orange uppercase tracking-widest">Mot de l'équipe pastorale</span>
                      </div>
